...
(define (fn-iter a b c count)
  (cond ((and (< count 3) (= a 2)) count) ; termination condition if inital value of n is < 3
        ((< count 3) a) ; termination condition if count > 2
        (else (fn-iter (+ a (* 2 b) (* 3 c)) a b (- count 1))) ; else iterate until count reaches termination condition
  ))
